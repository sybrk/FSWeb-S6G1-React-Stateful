...
export default function Programcilar() {
  ...
  const oneCikaninIsmi = () => {
    ...
    let result = programcilar.filter((item) => item.id === oneCikan)[0].isim
    return result;
  };
      ...
      <div className='programmers'>
        {
          ...
          programcilar.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => { setOneCikan(dev.id)/* burada dev.id 'yi öne çıkan id'ye atayın */ }}>Kutla</button>
            </div>
          )
        }
      </div>
